...
trait CommunicationModule: fmt::Debug {
    ...
    fn process_pulse(&mut self, module_id: usize, pulse: Pulse) -> Option<Pulse>;
...
impl CommunicationModule for Conjunction {
    ...
    fn process_pulse(&mut self, module_id: usize, pulse: Pulse) -> Option<Pulse> {
        self.memory.insert(module_id, pulse);
...
fn part_1(lines: Lines) -> usize {
    let (modules, module_destinations, broadcaster) = parse_input(lines);

    dbg!(&modules);
    dbg!(&module_destinations);
    dbg!(&broadcaster);
    0
}
